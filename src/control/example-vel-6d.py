...
if __name__ == '__main__':
    ...
    while (not rospy.is_shutdown()) and i < len(config):
        ...
        pose.pose.orientation.x = config[i][3]
        pose.pose.orientation.y = config[i][4]
        pose.pose.orientation.z = config[i][5]
        pose.pose.orientation.w = 0.0
        command_pose_pub.publish(pose)

        if rospy.Time.now().to_sec()-start_time >= config[i][6]:
            i += 1
            start_time = rospy.Time.now().to_sec()
