...
for start_time in start_times:
	...
	for topic, msg, t in bag.read_messages(topics='/measured_pose', start_time=start_time-rospy.rostime.Duration.from_sec(pre_time), end_time=start_time+rospy.rostime.Duration.from_sec(time)):
		...
		if len(x2) > 1:
			v1x.append((x2[-1]-x2[-2])/(msg.header.stamp.to_sec()-prev_x2_time))
			if len(v1x) > 1:
				a1x.append((v1x[-1]-v1x[-2])/(msg.header.stamp.to_sec()-prev_x2_time))
			prev_x2_time = msg.header.stamp.to_sec()
		if len(y2) > 1:
			v1y.append((y2[-1]-y2[-2])/(msg.header.stamp.to_sec()-prev_y2_time))
			if len(v1y) > 1:
				a1y.append((v1y[-1]-v1y[-2])/(msg.header.stamp.to_sec()-prev_y2_time))
			prev_y2_time = msg.header.stamp.to_sec()
		if len(z2) > 1:
			v1z.append((z2[-1]-z2[-2])/(msg.header.stamp.to_sec()-prev_z2_time))
			if len(v1z) > 1:
				a1z.append((v1z[-1]-v1z[-2])/(msg.header.stamp.to_sec()-prev_z2_time))
